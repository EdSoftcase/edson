...
import { 
  LayoutDashboard, DollarSign, LifeBuoy, FileText, 
  LogOut, Menu, X, Bell, Info 
} from 'lucide-react';
...
export const ClientLayout: React.FC = () => {
  ...
  const navItems = [
    { to: '/portal', label: 'Visão Geral', icon: LayoutDashboard, end: true },
    { to: '/portal/financeiro', label: 'Financeiro', icon: DollarSign, hidden: !portalSettings.allowInvoiceDownload },
    { to: '/portal/propostas', label: 'Propostas', icon: FileText },
    { to: '/portal/suporte', label: 'Suporte', icon: LifeBuoy, hidden: !portalSettings.allowTicketCreation },
  ];
  ...
  const myNotifications = useMemo(() => {
      ...
      return notifications
        .filter(n => n.relatedTo === currentClient.name || n.relatedTo === currentUser?.id)
                ...
                {navItems.filter(item => !item.hidden).map((item) => {
                  ...
                  return (
                    <NavLink
                      ...
                      `}
                      style={({ isActive }) => isActive ? { color: primaryColor, backgroundColor: `${primaryColor}10` } : {}}
                    >
              ...
              {navItems.filter(item => !item.hidden).map((item) => {
                 ...
                 return (
                  <NavLink
                    ...
                    className={({ isActive }) => `block w-full text-left px-3 py-3 rounded-md text-base font-medium flex items-center transition-colors ${
                      isActive
                        ? 'bg-slate-50'
